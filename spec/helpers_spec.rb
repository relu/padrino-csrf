...
describe Padrino::CSRF::Helpers do
  ...
  include Padrino::CSRF::AssetTagHelpers
  ...
  context "#button_to" do
	it 'should prepend the CSRF authenticity token to the form generated by button_to' do
	  button = button_to('Delete', '/delete')
	  button.should have_tag(:form, count: 1, with: { method: 'post', action: '/delete' }) do
		with_tag(:input, count: 1, with: { type: 'hidden', name: csrf_param, value: csrf_token })
		with_tag(:input, count: 1, with: { type: 'submit' })
	  end
	end
  end
end
